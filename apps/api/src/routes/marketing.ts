...
export async function registerMarketingRoutes(app: FastifyInstance, db: Db, env: AppEnv) {
  ...
  app.get("/api/v1/agents/tools", async () => ({
    tools: [
      ...
      { name: "search_companies", scope: "companies:read" },
      { name: "get_company", scope: "companies:read" },
      { name: "create_company", scope: "companies:create" },
      { name: "update_company", scope: "companies:update" },
